# Content Index

## Purpose

This index organizes the repo into a front door for executive transformation planning, modernization roadmaps, and governance alignment.

## Entry Points

- [Executive Roadmap Sequence](roadmaps/executive-roadmap-sequence.md)
- [Framework Overview](./framework-overview.md)
- [Transformation Strategy](./transformation-strategy.md)
- [Executive Governance](./executive-governance.md)
- [Platform Transformation](./platform-transformation.md)
- [Cloud Modernization Roadmaps](./cloud-modernization-roadmaps.md)
- [Operational Resilience Roadmaps](./operational-resilience-roadmaps.md)

## Transformation Models and Maturity

- [Business-Technology Alignment](../transformation-models/business-technology-alignment.md)
- [Governance-Driven Transformation](../transformation-models/governance-driven-transformation.md)
- [Transformation Maturity Model](../maturity-model/transformation-maturity-model.md)

## Roadmaps

- [Multi-Cloud Roadmap](../roadmaps/multi-cloud-roadmap.md)
- [Platform Engineering Roadmap](../roadmaps/platform-engineering-roadmap.md)
- [SRE Maturity Roadmap](../roadmaps/sre-maturity-roadmap.md)
- [DR Modernization Roadmap](../roadmaps/dr-modernization-roadmap.md)
- [FinOps Roadmap](../roadmaps/finops-roadmap.md)

## Templates

- [Executive Roadmap Template](../templates/executive-roadmap-template.md)
- [Quarterly Maturity Review](../templates/quarterly-maturity-review.md)
- [Transformation Plan Template](../templates/transformation-plan-template.md)

## References

- [Bibliography](../references/bibliography.md)
- [Transformation Impact Notes](../evidence/transformation-impact-notes.md)

## Shared Direction

Use the same section structure as the other core repos so the ecosystem stays consistent and easy to extend.

